...
class SignalCatcher:
    ...
    def catch(self):
        ...
        try:
            self.win = self.ahk.win_get(title="STALCRAFT")
    ...
    def searching(self):
        ...
        while self.run:
            ...
            self.logger.put("Проверка сигнала на обнаружение")
            for _ in range(6):
                if not self.run:
                    return
                QTest.qWait(250)
                ...
                if r == 255 and g == 255 and b == 255:
                    return
                self.logger.put(f"Цвет:  R[{r}] G[{g}] B[{b}]")
                if r + g + b > s_color_summ:
                    ...
                    if founded:
                        ...
                        return
            self.logger.put("Переоткрытие САК")
            self.reopen_suck()
    ...
    def burn_signal(self):
        ...
        if not self.run:
            return
        self.ahk.mouse_move(x=self.conf.x_med_rad, y=self.conf.y_med_rad, blocking=True)
        self.win.click(x=self.conf.x_med_rad, y=self.conf.y_med_rad, blocking=True)
        if not self.run:
            return
        # Запуск сканирования
        self.logger.put("Переключаю тумблер")
        self.ahk.mouse_move(x=self.conf.x_tumbler, y=self.conf.y_tumbler, blocking=True)
        ...
        if self.check_founded():
            ...
            self.ahk.mouse_move(x=self.conf.x_search, y=self.conf.y_search, blocking=True)
    ...
    def reopen_suck(self):
        ...
        if not self.run:
            return
        QTest.qWait(int(self.conf.reopen_time * 1000))
        if not self.run:
            return
        self.win.send("x")
